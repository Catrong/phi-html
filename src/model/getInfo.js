...
let infoPath = '/resources/info/'

async function fetchFile(path, branch = "main") {
    const url = `https://api.github.com/repos/catrong/phi-plugin/contents/${path}?ref=${branch}`;
    console.info(url)
...
class getInfo {
    ...
    */
    static getill(id, kind = 'common') {
        ...
        if (ans && !reg.test(ans)) {
            ans = `https://raw.githubusercontent.com/Catrong/phi-plugin/refs/heads/main/resources/otherill/${ans}`
            ...
            ans = `https://raw.githubusercontent.com/Catrong/phi-plugin-ill/refs/heads/main/${kk}/${id}.png`
        }
        if (!ans) {
    ...
    static getChapIll(name) {
        return `https://raw.githubusercontent.com/Catrong/phi-plugin-ill/refs/heads/main/chap/${name}.png`
    }
...
async function getFile() {
    ...
    let README = await fetchFile('README.md')
    let version = /https:\/\/img.shields.io\/badge\/插件版本-(.*)-9cf\?style=flat-square/.exec(README)[1]
